/*  THIS VERISON CUSTOMIZED ON SPAWN LOCATIONS AND WAYPOINT DETAILS!!!

    Spawn units at randomized locations (from list) and then send them moving on WPs
    This version scales to the number of players, so the RATIO of squads spawned to players is configured in the init box code used
        ...
        Syntax:
                handle = [
                                ...
                                ]  execVM "scripts\spawnTownies2.sqf";

                Example: handle = [this, ["spawnMarker1","spawnMarker2"], 6, 12, 1, "CAFme",["WP1a","WP1b","WP1c","WP1d","WP1e","WP1f","WP1z"]]  execVM "scripts\spawnTownies2.sqf";
...
//--SECTION 1 --------------------------------------------------------------------------------------------------
if (!isServer) exitwith {};//always, for spawning!
...
private ["_rndSpd","_rndBhv","_rndTO","_rndForm","_rndType","_rnd","_units","_squadInf","_HostileSquadMinBase","_HostileSqdMaxBase","_DifficultyFactor","_HostileSquadRange","_spawnPt","_size","_spawnee","_faction","_sqdRatio","_numSqds"];
        ...
        //Set the squad minimum size; athis is a hard bottom limit; total size will be randomized.
...
_numSqds = round( ((west countside allunits)/5) * _sqdRatio);
if (_numSqds <= 0) then {_numSqds = 1};

for "_n" from 1 to _numSqds do {
    hint format ["Townie Loop: %1",_n];//won't show on dedi server

    _spawnPt = (_this select 1) call BIS_fnc_selectRandom; //THIS SCRIPT IS SET UP TO USE [x,y,z] coords, not markers!
    ...
    _markerCount = (count _waypointMarkers) - 1;
    {
        ...
        _rndTO = 2+(random 5); //randomize Time Out

        // create and assign the waypoint
        if (_forEachIndex != _markerCount) then {

            _wp = _squadInf addWaypoint [getMarkerPos _markerName, 40]; //the number (default is 0) is the radius w/in which the WP will be, randomly
            if (_rndSpd < 0.6) then {_wp setWaypointSpeed "FULL"} ELSE {_wp setWaypointSpeed "NORMAL"}; // default is NORMAL
            if (_rndBhv < 0.7) then {_wp setWaypointBehaviour "SAFE"} ELSE {_wp setWaypointBehaviour "CARELESS"}; // default is AWARE ---I think
            if (_rndType < 0.1) then {_wp setWaypointType "SAD"};  //default is MOVE
            if (_rndForm < 0.6) then {_wp setWaypointFormation "FILE"} ELSE {_wp setWaypointFormation "STAG COLUMN"}; //default = WEDGE
            _wp setWaypointTimeout [0,_rndTO,(_rndTO + 5)];
    ...
    sleep 120+(random 120); //put a pause between each squad spawning
